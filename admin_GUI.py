        ...
        def show_others():
            ...
            def on_select(event):
                if "disabled" in users_treeview.item(users_treeview.focus(), "tags"):
                    ...
                    tk.Button(edituserframe, text="Confirm Changes", bg='#b3794a', width = 11,
                              ...
                              command=lambda: UserManager.edit_user(username, edited_username.get(), edited_password.get(),users_treeview, edituserframe)).grid(row=4, column=1, padx=100, pady =30, sticky = 'e', columnspan=2)
                    tk.Button(edituserframe, text=  "Delete User", bg='#b3794a', font = ("Comic Sans MS", 15, "bold"), width = 11, command = lambda: UserManager.del_user(username, users_treeview, edituserframe)).grid(row=3, column = 0, sticky = 'w',  padx=100, columnspan=2)
                    tk.Button(edituserframe, text="Cancel", command=edituserframe.destroy,  width = 11, font=("Comic Sans MS", 15, "bold")).grid(row=4, column=0, sticky = 'w', pady=53, padx=100, columnspan=2)
                    if select_row[3] == "True":
                        tk.Button(edituserframe, text="Disable User" ,  width = 11,
                              font=("Comic Sans MS", 15, "bold"), command = lambda: UserManager.disable_user(username, edituserframe, users_treeview)).grid(row=3, column=1, padx=100, sticky = 'e', columnspan=2)
                    elif select_row[3] == "False":
                        tk.Button(edituserframe, text="Enable User" , width = 11,font=("Comic Sans MS", 15, "bold"), command = lambda: UserManager.enable_user(username, edituserframe, users_treeview)).grid(row=3, column=1, padx=100, sticky = 'e', columnspan=2)
                    for i in range(4):
           ...
           #Creates GUI For Messaging System
            ...
            tk.Button(self.adduser_subframe, text="Create User", bg = '#b3794a', font = ("Comic Sans MS", 18, "bold"),
                       command=lambda: UserManager.create_user(new_username.get(), new_password.get().lower(), user_role.get().lower(), users_treeview)).grid(row=3, columnspan=2)
