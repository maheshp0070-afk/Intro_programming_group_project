...
def show_log_coord(login_root):
    ...
    def show_others(): #maybe also add a key above the map for tent icons
        ...
        #msging system here:
        MessagingApp(msgsubframe, coordinator.username)
        ...
        tk.Label(header, text="Logistics Coordinator", font=("Comic Sans MS", 18), background='#1095d6', fg="white").grid(row=0, column=0,
                                                                                            ...
                                                                                            pady=10)
        global home_button
        home_button = ttk.Button(header, text="Back to Dashboard", command=lambda: back_to_dashboard())
        home_button.grid(row=0, column=1, sticky='e', padx=10, pady=10)
        ...
        ttk.Button(header, text="Logout",  command = lambda: logout() if messagebox.askyesno("Logout","Are you sure you want to logout?") else None).grid(row=0, column=2, sticky='e', padx=10, pady=10)

    def back_to_dashboard():
        root2.destroy()
        show_log_coord(login_root)        
    ...
    def show_edit_camp(camp, state):
        ...
        camp_activities = df_activities[df_activities['camp_name'] == camp.name]

        for index, activity in camp_activities.iterrows():
            assigned = str(activity['assigned_campers']).strip()
            num_assigned = len(assigned.split(',')) if assigned and assigned != '' else 0
            max_cap = int(activity['max_capacity'])
            engagement_pct = (num_assigned / max_cap) * 100 if max_cap > 0 else 0
            
            if activity['activity_name'] == 'Hiking':
                hiking_engagement_bar['value'] = engagement_pct
            elif activity['activity_name'] == 'Archery':
                archery_engagement_bar['value'] = engagement_pct
            elif activity['activity_name'] == 'Rock Climbing':
                rockclimbing_engagement_bar['value'] = engagement_pct
            elif activity['activity_name'] == 'Campfire':
                campfire_engagement_bar['value'] = engagement_pct
            ...
            if camp_name_input == "":
                ...
                if selected_sdate > selected_edate:
                    ...
                    for index, row in df_activities.iterrows():
                        ...
                        if row['camp_name'] == old_name:
                            df_activities.at[index, 'camp_name'] = camp.name
        ...
        if state == "completed":
            ...
            block_interactions(stock_spinbox)
            stock_spinbox.configure(cursor="X_cursor")
