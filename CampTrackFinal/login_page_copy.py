...
class LoginPage(tk.Frame):
    def __init__(self, master=None):
        ...
        self.login = ttk.Entry(self.canvas, width=40, font=("Comic Sans MS", 20), validate="key",
                               validatecommand=(self.register(lambda x: len(x) < 20), '%P'))
        self.password = ttk.Entry(self.canvas, width=40, show="*", font=("Comic Sans MS", 20), validate="key",
                                  validatecommand=(self.register(lambda x: len(x) < 20), '%P'))
    ...
    def login_(self):
        ...
        if status == True:
            if role == "coordinator":
                ...
                root.withdraw()
                from log_coord_dashboard_copy import show_log_coord
                show_log_coord(root)
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("CampTrack Login Page")
