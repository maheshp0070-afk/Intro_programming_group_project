...
class Camp:
    ...
    def __init__(self, name, location, camp_type, start_date, end_date, food_supply_per_day, food_demand_per_day, leader, pay, food_requirement_per_camper, topped_up):
       ...
       self.leader = leader
       self.pay = pay
       self.food_requirement_per_camper = food_requirement_per_camper
    ...
    def load_camps (cls, file):
       ...
       for index, row in df_camps.iterrows():
           ...
           camp = Camp(
               ...
               leader = row['leader'],
               pay = int(row['pay']),
               food_requirement_per_camper = int(row['food_requirement_per_camper']),
               topped_up = int(row['topped_up'])
           )
    ...
    def save_camps(file):
       ...
       for camp in camps.values():
           ...
           camp_data.append({
               ...
               'leader': camp.leader,
               'pay': int(camp.pay),
               'food_requirement_per_camper': camp.food_requirement_per_camper,
               'topped_up': int(camp.topped_up)
           })
...
class Coordinator(User):
    ...
    def create_camp(self, camps_dict, name, location, camp_type, start_date, end_date, food_supply_per_day, pay, topped_up):
        ...
        new_activities = [
        {'activity_id': 1, 'camp_name': name, 'activity_name': 'Hiking', 'start_time': '09:00', 'end_time': '12:00', 'max_capacity': 20, 'assigned_campers': '', 'extra_notes': ''},
        {'activity_id': 2, 'camp_name': name, 'activity_name': 'Archery', 'start_time': '14:00', 'end_time': '16:00', 'max_capacity': 20, 'assigned_campers': '', 'extra_notes': ''},
        {'activity_id': 3, 'camp_name': name, 'activity_name': 'Campfire', 'start_time': '19:00', 'end_time': '21:00', 'max_capacity': 20, 'assigned_campers': '', 'extra_notes': ''},
        {'activity_id': 4, 'camp_name': name, 'activity_name': 'Rock Climbing', 'start_time': '08:00', 'end_time': '17:00', 'max_capacity': 20, 'assigned_campers': '', 'extra_notes': ''}
        ]
    
        df_activities = pd.concat([df_activities, pd.DataFrame(new_activities)], ignore_index=True)
