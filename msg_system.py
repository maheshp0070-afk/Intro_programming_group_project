...
class MessagingApp:
    def __init__(self, container, logged_in_username):
        """Initialise a messaging widget in a Tk root, toplevel, or embedded frame."""
        self.root = container
        self._host = container if isinstance(container, (tk.Tk, tk.Toplevel)) else container.winfo_toplevel()
        if isinstance(container, (tk.Tk, tk.Toplevel)) and hasattr(container, "geometry"):
            container.geometry("700x400")
    ...
    def create_widgets(self):
        ...
        ttk.Button(self.right, text="Delete Conversation", command=self.delete_conv).grid(row=3, column=0)
    ...
    def update_receiver_options(self, *args):
        ...
        if self._host is not None and hasattr(self._host, "title"):
            self._host.title("Messaging App - User: " + sender_id)
...
if __name__ == '__main__':
    ...
    MessagingApp(root, logged_in_username="scoutleader")
    root.mainloop()
