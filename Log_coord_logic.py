...
class Camp:
   ...
   def __init__(self, name, location, type, start_date, end_date, food_supply_per_day, food_demand_per_day, leader, pay):
   ...
   def get_campers(self, campers_file):
       ...
       campers = [row['camper_id'] for index, row in df_campers.iterrows() if type(row['camps']) == str if self.name in row['camps'].split(',')]
